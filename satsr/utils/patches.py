...
def interp_patches(lr_image, hr_image_shape):
    interp = np.zeros(
        (lr_image.shape[0:2] + hr_image_shape[2:4])
    ).astype(np.float32)
...
def upsample_patches(lr_patch, output_shape):
    ...
    """
    up_patches = np.zeros((lr_patch.shape[0], *output_shape)).astype(
        np.float32
    )
...
def get_test_patches(
    data_bands, patch_size=128, border=4, interp=True
):
    ...
    if not np.mod(P_i, Q_i) == 0:
        range_i = np.append(
            range_i,
            padded_bands[max_res].shape[0] - patch_sizes[max_res][0],
        )
    if not np.mod(P_j, Q_j) == 0:
        range_j = np.append(
            range_j,
            padded_bands[max_res].shape[1] - patch_sizes[max_res][0],
        )

    # Save the patches
    images = {
        res: np.zeros(
            (nr_patches, padded_bands[res].shape[2])
            + patch_sizes[res]
        ).astype(np.float32)
        for res in resolutions
    }
    ...
    for ii in range_i.astype(int):
        for jj in range_j.astype(int):
            ...
            for res in resolutions:
                tmp_crop_point = [
                    p * inv_scales[res] for p in crop_point
                ]
    ...
    if interp:
        for res in resolutions:
            images[res] = interp_patches(
                images[res], images[min_res].shape
            )
...
def save_random_patches(gt, lr, save_path, num_patches=None):
    ...
    for i, crop in enumerate(tqdm(range(0, num_patches))):
        ...
        while not found:
            ...
            upper_left_x = randrange(
                0, lr[max_res].shape[0] - PATCH_SIZE_LR[0]
            )
            upper_left_y = randrange(
                0, lr[max_res].shape[1] - PATCH_SIZE_LR[1]
            )
            crop_point_lr = [
                upper_left_x,
                upper_left_y,
            ...
            tmp_label = np.moveaxis(
                tmp_label, source=2, destination=0
            )  # move to channels first
            ...
            if count == 100:
                print(
                    "Error while taking patches from the image: it looks like the image is mostly made of zeros. "
                    "Skipping the tile ..."
                )
                return None
        ...
        for res in lr.keys():
            ...
            tmp_images[res] = np.moveaxis(
                tmp_images[res], source=2, destination=0
            )  # move to channels first
        ...
        for res in lr.keys():
            tmp_images[res] = upsample_patches(
                tmp_images[res],
                output_shape=tmp_images[min_res].shape[1:],
            )
...
def recompose_images(a, border, size=None):
    ...
    if a.shape[0] == 1:
        ...
        # Initialize image
        images = np.zeros((a.shape[1], size[0], size[1])).astype(
            np.float32
        )
        current_patch = 0
        for y in range(0, y_tiles):
            ...
            for x in range(0, x_tiles):
                ...
                images[
                    :,
                    ypoint : ypoint + patch_size,
                    xpoint : xpoint + patch_size,
                ] = a[
                    current_patch,
                    :,
