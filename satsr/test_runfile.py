...
def load_models():
    ...
    for res in resolutions:
        if (
            res == min_res
        ):  # no need to build a model for the minimum resolution
            continue
        input_shape = {
        ...
        models[res] = load_model(
            ...
            modelname="{}_model_{}m".format(models_sat, res),
        )
...
def test(
    ...
    if (
        max_res is None
    ):  # we super-resolve for all possible resolutions
    ...
    if output_path is None:
        ...
        output_path = os.path.join(
            paths.get_test_dir(), "outputs", output_name
        )

    if (
        output_file_format == "ENVI"
        and output_path[-4:].lower() == ".hdr"
    ):
        output_path = (
            output_path[:-4] + ".bin"
        )  # ENVI file name should be the .bin, not the .hdr

    if not gdal_utils.check_gdal_format(output_file_format):
        print(
            "GDAL doesn't support creating %s files"
            % output_file_format
        )
    ...
    for res in sr_bands.keys():
        ...
        tmp_bands = {
            tmp_res: bands
            for tmp_res, bands in data_bands.items()
            if tmp_res <= res
        }
    ...
    mean_mask = np.prod(
        mask[min_res], axis=2
    )  # most restrictive mask at the min_resolution (in case masks of bands at minimum are different).
    for res, bands in sr_bands.items():
        if (
            res % min_res
        ) == 0:  # resolutions are multiples of one another
            scale = int(res / min_res)
            new_mask = np.repeat(
                np.repeat(mask[res], repeats=scale, axis=0),
                repeats=scale,
                axis=1,
            )
        else:
    ...
    if output_file_format == "npz":
        output_dict = dict(zip(output_shortnames, output_bands))
