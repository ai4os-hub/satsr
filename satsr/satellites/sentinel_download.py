...
class Satellite:
    ...
    def search(self, omit_corners=True):
        ...
        query = {
            "footprint": '"Intersects(POLYGON(({0} {1},{2} {1},{2} {3},{0} {3},{0} {1})))"'.format(
                self.coord["W"],
                self.coord["S"],
                self.coord["E"],
                self.coord["N"],
            ...
            "beginposition": "[{} TO {}]".format(
                self.inidate, self.enddate
            ),
        }
        ...
        print(
            "Found {} results".format(
                json_feed["opensearch:totalResults"]
            )
        )
    ...
    def download(self, results, output_folder):
        ...
        for r in results:
            ...
            save_dir = os.path.join(
                output_folder, "{}.SAFE".format(tile_id)
            )
