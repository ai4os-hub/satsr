...
def read_bands(
    ...
    if (
        os.path.basename(xml_path) == "MTD_MSIL2A.xml"
        and gdal_ver < "2.4.0"
    ):
        raise Exception(
            "Sentinel2 L2A is temporarily not supported due to version conflicts in the base Docker image"
        )
    ...
    for tmidx, (dsname, dsdesc) in enumerate(
        sets[10] + sets["unknown"]
    ):
        ds = gdal.Open(dsname)
    ...
    ds_bands = {
        res: gdal.Open(selected_dataset[res][0])
        for res in selected_dataset.keys()
    }
    ...
    for res in ds_bands.keys():
        ...
        for b in range(0, ds_bands[res].RasterCount):
            desc = get_short_description(
                ds_bands[res].GetRasterBand(b + 1).GetDescription()
            )
    ...
    for res in validated_bands.keys():
        assert validated_bands[res] == res_to_bands[res], (
            "Error loading bands at {}m resolution. \n"
            "Expected band list: {} \n"
            "Actual band list: {}".format(
                res, res_to_bands[res], validated_bands[res]
            )
        )
    ...
    for res in validated_bands.keys():
        print(
            "Loading selected data from: {}m".format(
                selected_dataset[res][1]
            )
        )
        uf = upscaling_factor[res]
        data_bands[res] = ds_bands[res].ReadAsArray(
            xoff=xmin // uf,
            yoff=ymin // uf,
        ...
        data_bands[res] = np.moveaxis(
            data_bands[res], source=0, destination=-1
        )  # move to channels last
        data_bands[res] = data_bands[res][
            :, :, validated_indices[res]
        ]
