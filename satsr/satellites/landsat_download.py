...
class Satellite:
    def __init__(
        ...
        # API
        api_version = "1.4.1"
        self.api_url = "https://earthexplorer.usgs.gov/inventory/json/v/{}/".format(
            api_version
        )
        ...
        if json_feed["error"]:
            raise Exception(
                "Error while searching: {}".format(json_feed["error"])
            )
    ...
    def search(self):
        ...
        query = {
            ...
            "spatialFilter": {
                "filterType": "mbr",
                "lowerLeft": {
                    "latitude": self.coord["S"],
                    "longitude": self.coord["W"],
                },
                "upperRight": {
                    "latitude": self.coord["N"],
    ...
    def download(self, results, output_folder):
        ...
        data = {
            ...
            "__ncforminfo": re.findall(
                r'name="__ncforminfo" value="(.+?)"', response.text
            ),
        }
        response = self.session.post(
            self.login_url, data=data, allow_redirects=False
        )
        response.raise_for_status()
        ...
        for r in results:
            ...
            url = "https://earthexplorer.usgs.gov/download/12864/{}/STANDARD/EE".format(
                tile_id
            )
