...
band_desc = {
    "B1": "B1 Ultra Blue (coastal/aerosol) [435nm-451nm]",
...
def read_config_file(filepath):
    ...
    for line in f:
        line = line.lstrip(
            " "
        ).rstrip()  # remove leading whitespaces and trainling newlines
        ...
        if line.startswith("GROUP"):
            ...
            try:
                set_by_path(
                    root=config,
                    items=group_path + [key],
                    value=json.loads(value),
                )
            except Exception:
                set_by_path(
                    root=config, items=group_path + [key], value=value
                )
    f.close()
    return config
...
if __name__ == "__main__":
    read_bands(
        tile_path="/media/ignacio/Datos/datasets/satelites/LandSat_tiles/LC80200362019035LGN00",
