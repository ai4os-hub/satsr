...
class Satellite:
    def __init__(
        ...
        self.credentials = {
            "username": username,
            "password": password,
        }  # no credentials seem to be needed
    ...
    def search(self):
        ...
        response = self.session.get(
            self.api_url + "searchForFiles", params=query
        )
        ...
        response = self.session.get(
            self.api_url + "getFileUrls", params=query
        )
        resp_xml = ET.fromstring(response.text)
        ...
        query = {"fileIds": ",".join(id_list)}
        response = self.session.get(
            self.api_url + "getFileProperties", params=query
        )
        resp_xml = ET.fromstring(response.text)
    ...
    def download(self, results, output_folder):
        ...
        for r in results:
            ...
            with urlopen(r["url"]) as in_stream, open(
                filepath, "wb"
            ) as out_file:
