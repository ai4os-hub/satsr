...
# [1] https://packages.ubuntu.com/search?keywords=libgdal-dev
...
def train_fn(TIMESTAMP, CONF):
    ...
    if CONF["training"]["max_res"] is None:
        CONF["training"]["max_res"] = max(
            main_sat.res_to_bands().keys()
        )
    ...
    input_shape = {
        tmp_res: default_shapes[tmp_res]
        for tmp_res in default_shapes.keys()
        if int(tmp_res) <= int(max_res)
    }
    ...
    train_tiles = data_utils.load_data_splits(
        splits_dir=paths.get_splits_dir(), split_name="train"
    )
    ...
    if CONF["training"]["overwrite"]:
        ...
        for tile_name in os.listdir(paths.get_patches_dir()):
            ...
            tile_path = os.path.join(
                paths.get_patches_dir(), tile_name
            )
            shutil.rmtree(tile_path)

        print("Creating train patches ...")
        data_utils.create_patches(
            tiles=train_tiles,
    ...
    train_gen = data_utils.data_sequence(
        tiles=train_tiles,
    ...
    if "val.txt" in os.listdir(paths.get_splits_dir()):
        val_tiles = data_utils.load_data_splits(
            splits_dir=paths.get_splits_dir(), split_name="val"
        )
        ...
        val_gen = data_utils.data_sequence(
            tiles=val_tiles,
            batch_size=CONF["training"]["batchsize"],
            patches_dir=paths.get_patches_dir(),
            max_res=max_res,
    ...
    with open(
        os.path.join(paths.get_conf_dir(), "model.yaml"), "w"
    ) as yaml_file:
